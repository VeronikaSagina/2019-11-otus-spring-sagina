...
class BookServiceTest {
    ...
    void createBookTest() {
        ...
        Mockito.when(genreRepository.findAllByIdIn(created.getGenreIds())).thenReturn(List.of(GenreData.FANTASTIC, GenreData.NOVEL));
        Mockito.when(bookRepository.save(Mockito.any())).thenReturn(BookData.SNUFF);

        Book expected = new Book();
        expected.setTitle(created.getTitle());
        ...
        Assertions.assertEquals(BookData.SNUFF.getAuthor(), actual.getAuthor());
    ...
    void updateBookTest() {
        ...
        Mockito.when(bookRepository.findById(updateBook.getId())).thenReturn(Optional.of(BookData.getBookForAnnaKarenina()));
        Mockito.when(authorRepository.findById("1")).thenReturn(Optional.of(AuthorData.TOLSTOY));
        Mockito.when(genreRepository.findAllByIdIn(updateBook.getGenreIds()))
    ...
    void getBookTest() {
        ...
        Assertions.assertEquals(BookData.ANNA_KARENINA.getId(), actual.getId());
        Assertions.assertEquals(BookData.ANNA_KARENINA.getTitle(), actual.getTitle());
        Assertions.assertEquals(BookData.ANNA_KARENINA.getAuthor(), actual.getAuthor());
    }
}
