...
class AuthorServiceTest {
    ...
    void createAuthorTest() {
        ...
        Mockito.verify(authorRepository).save(new Author(null, AuthorData.PUSHKIN.getName(), List.of()));
    }

    @Test
    void updateAuthorTest() {
        Mockito.when(authorRepository.findById(1)).thenReturn(Optional.of(AuthorData.PELEVIN));
        Mockito.when(authorRepository.save(Mockito.any())).thenReturn(new Author(1, AuthorData.PELEVIN.getName(), null));
