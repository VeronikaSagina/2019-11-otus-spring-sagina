...
class BookServiceTest {
    ...
    @Test
    void deleteBooksByAuthorIdTest() {
        Author author = new Author(AuthorData.TOLSTOY.getId(), AuthorData.TOLSTOY.getName());
        Mockito.when(authorRepository.findById(author.getId())).thenReturn(Optional.of(author));
        Mockito.when(bookRepository.findAllByAuthorIdIn(List.of(author.getId())))
                .thenReturn(List.of(BookData.ANNA_KARENINA));
        bookService.deleteBooksByAuthorId(author.getId());
        Mockito.verify(bookRepository).deleteAll(List.of(BookData.ANNA_KARENINA));
    }
    ...
    @Test
    void getAllBooksByAuthorNameTest() {
