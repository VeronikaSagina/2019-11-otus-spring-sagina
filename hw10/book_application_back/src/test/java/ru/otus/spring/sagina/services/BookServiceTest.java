...
class BookServiceTest {
    ...
    @Test
    void createBookTest() {
        BookDtoRequest created = new BookDtoRequest("", BookData.SNUFF.getName(), BookData.DESCRIPTION,
                AuthorDtoMapper.toDto(AuthorData.PELEVIN),
        ...
        Mockito.when(genreRepository.findAllByIdIn(created.genreIds))
                .thenReturn(List.of(GenreData.FANTASTIC, GenreData.NOVEL));
        Mockito.when(bookRepository.save(Mockito.any())).thenReturn(BookData.SNUFF);

        Book expected = new Book();
        expected.setName(created.name);
        ...
        Assertions.assertEquals(BookData.SNUFF.getName(), actual.getName());
    }

    @Test
    void updateBookTest() {
        BookDtoRequest updateBook = new BookDtoRequest("1", BookData.ANNA_KARENINA.getName(),
                BookData.DESCRIPTION, AuthorDtoMapper.toDto(AuthorData.TOLSTOY), List.of("1", "2"));
        ...
        Assertions.assertEquals(BookData.ANNA_KARENINA.getName(), actual.getName());
